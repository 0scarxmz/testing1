...
function getDatabase() {
  ...
  try {
    sqliteInstance.exec(`
      ALTER TABLE notes ADD COLUMN icon_path TEXT;
    `);
  } catch (e) {
    // Column already exists, ignore
  }
  ...
  try {
    sqliteInstance.exec(`
      ALTER TABLE notes ADD COLUMN icon_emoji TEXT;
    `);
  } catch (e) {
    // Column already exists, ignore
  }
...
export async function getAllNotes() {
  ...
  const results = await db.select({
    ...
    iconPath: notes.iconPath,
    iconEmoji: notes.iconEmoji,
...
export async function createNote(data: {
  ...
  iconPath?: string | null; // path to custom note icon file
  iconEmoji?: string | null; // custom emoji for note icon
...
export async function updateNote(id: string, data: {
  ...
  // Explicitly handle iconPath
  if (data.iconPath !== undefined) {
    updateData.iconPath = data.iconPath || null;
  }

  // Explicitly handle iconEmoji
  if (data.iconEmoji !== undefined) {
    updateData.iconEmoji = data.iconEmoji || null;
  }
...
export async function getChildNotes(parentId: string) {
  ...
  const results = await db.select({
    ...
    status: notes.status,
    priority: notes.priority,
  }).from(notes).where(eq(notes.parentId, parentId)).all();
  return results;
}
