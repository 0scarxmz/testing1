...
async function processNoteAsync(noteId: string, content: string) {
  ...
  try {
    console.log('[storage-sqlite] Starting async processing for note', noteId);
    ...
    if (desktopAPI.captureScreenshot) {
      desktopAPI.captureScreenshot(noteId).then(screenshotPath => {
        if (screenshotPath) {
          console.log('[storage-sqlite] Screenshot captured for note', noteId);
    ...
    console.log('[storage-sqlite] Generated:', { title, tags, hasEmbedding: !!embedding });
    ...
    console.log('[storage-sqlite] Successfully updated note', noteId, 'with AI-generated content');
  } catch (error) {
